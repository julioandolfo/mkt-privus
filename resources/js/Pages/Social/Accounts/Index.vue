...
const POSTIZ_PLATFORMS = ['linkedin', 'linkedin_page', 'tiktok', 'google_my_business'];
function isPostizPlatform(platform: string): boolean {
    return POSTIZ_PLATFORMS.includes(platform);
}
...
function connectOAuth(platform: string) {
    ...
    const usePostiz = isPostizPlatform(platform);
    const redirectRoute = usePostiz ? 'social.postiz.oauth.redirect' : 'social.oauth.redirect';

    fetch(route(redirectRoute, platform) + '?' + params.toString(), {
        headers: {
        ...
        if (data.url) {
            ...
            const checkClosed = setInterval(() => {
                if (popup && popup.closed) {
                    ...
                    if (usePostiz) syncPostizIntegrations();
                }
            }, 500);
        } else {
            connectingPlatform.value = null;
...
async function syncPostizIntegrations() {
    try {
        showToast('Sincronizando contas com Postiz...', 'info');
        const response = await axios.post(route('social.postiz.integrations.sync'));
        if (response.data.success) {
            showToast(response.data.message || 'Contas sincronizadas!', 'success');
            router.reload({ only: ['accounts'] });
        } else {
            showToast(response.data.message || 'Falha ao sincronizar.', 'error');
        }
    } catch (e: any) {
        const msg = e?.response?.data?.message || 'Erro ao sincronizar com Postiz.';
        showToast(msg, 'error');
    }
}
...
const platformInfo: Record<string, { desc: string; features: string[] }> = {
    ...
    linkedin: {
        desc: 'Perfil profissional (via Postiz).',
        features: ['Publicar no perfil', 'Agendar posts', 'Gerenciado via Postiz'],
    },
    linkedin_page: {
        desc: 'Pagina de empresa no LinkedIn (via Postiz).',
        features: ['Publicar na pagina', 'Carrossel', 'Gerenciado via Postiz'],
    },
    tiktok: {
        desc: 'Publicar videos no TikTok (via Postiz).',
        features: ['Publicar videos', 'Configurar privacidade', 'Gerenciado via Postiz'],
    },
    google_my_business: {
        desc: 'Posts e novidades do estabelecimento (via Postiz).',
        features: ['Posts locais', 'Ofertas', 'Gerenciado via Postiz'],
    },
};
...
const platformIcons: Record<string, string> = {
    ...
    google_my_business: 'M12.24 10.285V14.4h6.806c-.275 1.765-2.056 5.174-6.806 5.174-4.095 0-7.439-3.389-7.439-7.574s3.345-7.574 7.439-7.574c2.33 0 3.891.989 4.785 1.849l3.254-3.138C18.189 1.186 15.479 0 12.24 0c-6.635 0-12 5.365-12 12s5.365 12 12 12c6.926 0 11.52-4.869 11.52-11.726 0-.788-.085-1.39-.189-1.989H12.24z',
};
                            ...
                            <div class="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-800/30 transition">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span v-if="account.source === 'postiz'" class="rounded-full bg-purple-500/10 border border-purple-500/30 px-2 py-0.5 text-[10px] text-purple-300" title="Gerenciado via Postiz">via Postiz</span>
                                    </div>
                                ...
                                <div class="flex items-center gap-1.5 shrink-0">
                                    ...
                                    <button v-if="account.source !== 'postiz'" @click="syncAccountInsights(account)"
